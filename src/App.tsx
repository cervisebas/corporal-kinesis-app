...
const App = ()=>{
    ...
    Account.verify().then((value)=>{
        if (value) {
            var dataUser: any = value;
            setTextVerify(`Accediendo como "${decode(dataUser.email).slice(0, 5)}...${decode(dataUser.email).slice(decode(dataUser.email).indexOf('@'), decode(dataUser.email).length)}"`);
        }
        setTimeout(() => {
            setOpenSession(!value);
            if (value) setLoadNow(true);
            setShowVerify(false);
        }, 2500);
    }).catch((error)=>{
        console.log(error);
        Alert.alert('Ocurrio un error', String(error));
    });
    ...
    return(<View style={{ flex: 1, marginTop }}>
