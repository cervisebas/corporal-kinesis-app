...
export type ViewMoreDetailsRef = {
    open: (dataShow: DetailsTrainings, commentData?: commentsData)=>void;
    close: ()=>void;
};
...
export default React.memo(forwardRef(function ViewMoreDetails(_props: any, ref) {
    // Context's
    ...
    // Controller
    function open(_dataShow: DetailsTrainings, _commentData?: commentsData) {
    ...
    return(<CustomModal visible={visible} onRequestClose={close}>
        <View style={{ flex: 1, backgroundColor: theme.colors.background }}>
            <Appbar.Header style={{ backgroundColor: theme.colors.background }}>
                ...
                <Appbar.Content title={`Más detalles (${(dataShow)? dataShow.date.value: ''})`}/>
            </Appbar.Header>
            {(dataShow)&&<ScrollView style={{ flex: 2, paddingBottom: 16 }}>
