...
export default class CommentSystem {
            ...
            AsyncStorage.getItem('account_session').then((value)=>{
                ...
                axios.post(`${this.urlBase}/index.php`, QueryString.stringify({ getAllComments: true, email: datas.email, password: datas.password }), this.header_access).then((html)=>{
                    try {
                        ...
                        if (result.ok) {
                            var res: any = result.data?.reverse();
                            resolve(res);
                        } else {
    ...
    admin_create(idAccount: string, comment: string, idTraining?: string): Promise<boolean> {
        return new Promise((resolve, reject)=>{
            AsyncStorage.getItem('account_session').then((value)=>{
                if (!value) return reject({ cause: 'No se ha encontrado datos de inicio de sesión.', action: 1, error: false });
                var datas: storageData = JSON.parse(decode(String(value)));
                var dataPost = {
                    ...
                    idAccount,
                    comment,
                    date: encode(moment(new Date()).format('DD/MM/YYYY HH:mm[hs]')),
                    idTraining
                };
    ...
    admin_delete(idComment: string): Promise<boolean> {
        return new Promise((resolve, reject)=>{
            AsyncStorage.getItem('account_session').then((value)=>{
                if (!value) return reject({ cause: 'No se ha encontrado datos de inicio de sesión.', action: 1, error: false });
                var datas: storageData = JSON.parse(decode(String(value)));
                var dataPost = { deleteAdminComment: true, email: datas.email, password: datas.password, idComment };
                axios.post(`${this.urlBase}/index.php`, QueryString.stringify(dataPost), this.header_access).then((html)=>{
                    try {
                        var result: tipical = html.data;
                        if (result.ok) resolve(true); else reject({ cause: (result.cause?.length !== undefined && result.cause?.length !== 0)? result.cause: 'Ocurrió un error inesperadamente.', error: false });
    ...
    admin_modify(idComment: string, comment: string): Promise<boolean> {
        return new Promise((resolve, reject)=>{
            AsyncStorage.getItem('account_session').then((value)=>{
                if (!value) return reject({ cause: 'No se ha encontrado datos de inicio de sesión.', action: 1, error: false });
                var datas: storageData = JSON.parse(decode(String(value)));
                var dataPost = { editAdminComment: true, email: datas.email, password: datas.password, idComment, comment, date: encode(moment(new Date()).format('DD/MM/YYYY HH:mm[hs]')) };
                axios.post(`${this.urlBase}/index.php`, QueryString.stringify(dataPost), this.header_access).then((html)=>{
                    try {
                        var result: tipical = html.data;
                        if (result.ok) resolve(true); else reject({ cause: (result.cause?.length !== undefined && result.cause?.length !== 0)? result.cause: 'Ocurrió un error inesperadamente.', error: false });
    ...
    admin_getAllAccount(idAccount: string): Promise<commentsData[]> {
        return new Promise((resolve, reject)=>{
            AsyncStorage.getItem('account_session').then((value)=>{
                if (!value) return reject({ cause: 'No se ha encontrado datos de inicio de sesión.', action: 1, error: false });
                var datas: storageData = JSON.parse(decode(String(value)));
                var dataPost = { getAdminAllComments: true, email: datas.email, password: datas.password, idAccount };
                axios.post(`${this.urlBase}/index.php`, QueryString.stringify(dataPost), this.header_access).then((html)=>{
                    try {
                        var result: getCommentsAll = html.data;
                        if (result.ok) {
